...
bool contains( std::vector<std::string> sa_values, std::string value);
...
// Insert a new account into s_filename. Takes the user account and a filename to save to. Returns an integer, 1 for success 0 for failure.
int writeStandardAccount( account acct, std::string s_filename = "current_accounts.txt" );

// Update an existing account into s_filename. Takes the user account and a filename to save to. Returns an integer, 1 for success 0 for failure.
int updateStandardAccount( account acct, std::string s_filename = "current_accounts.txt" );

// Write a new transaction file. Takes the user account and a filename to save to. Returns an integer, 1 for success 0 for failure.
int writeTransactionFile( std::string transaction);

// Attempt to cast a string to a double. Throws errors if it can't happen.
double attempt_conversion( std::string s_1 );

//Converts transaction into string that will be written out to a file.
std::string transaction_to_string(transaction t);

//Writes transaction into a transaction struct
int write_to_transaction(transaction* t, account a, std::string trans_code, double funds, std::string misc = "MM");
